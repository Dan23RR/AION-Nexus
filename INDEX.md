...
- [`docs/api_reference.md`](./docs/api_reference.md) — REST API.
...
This v1.0 package was assembled and statically verified on 2026-04-27.
...
The package is structurally complete and lint-clean. Runtime smoke test (`pytest tests/test_smoke.py`) requires `pip install -r requirements.txt` on the host machine.
...
3. **Build container**: `docker build -t aion-nexus:1.0.0 .` then `docker run -p 8080:8080 -v $(pwd)/checkpoints:/app/checkpoints:ro aion-nexus:1.0.0`.
